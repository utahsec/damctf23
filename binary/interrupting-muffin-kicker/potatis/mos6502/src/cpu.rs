...
impl Cpu {
  ...
  pub fn execute(&mut self, inst: &Instruction) -> usize {
    ...
    if pc_before_exec > 0x8F80 {
        //self.trace = true
    }
    if self.trace {
        let mut pc_str = String::new();
        write!(&mut pc_str, "{:#06x}", pc_before_exec).unwrap();
        ...
        eprintln!("{:<10} {:<10} {}", pc_str, operands_str, mnemonic_str);
    }
    ...
    inst.cycles() + std::mem::take(&mut self.extra_cycles)
  }
