...
impl Server {
  ...
  pub fn serve(mut self) -> Result<(), Box<dyn Error>> {
    let servers = [
      ("./included-roms/muffin_kicker.nes", self.settings.easy_port),
      ("./included-roms/interrupting_muffin_kicker.nes", self.settings.hard_port),
    ];

    let (tx, rx) = std::sync::mpsc::channel();
    for (rom, port) in servers.into_iter() {
      let host = format!("{}:{}", self.settings.host, port);
      info!("{:?} listening on {}", rom, host);
      let server = TcpListener::bind(host)?;
      Self::start_accepting(server, tx.clone(), rom);
    };
  ...
  fn start_accepting(srv_socket: TcpListener, tx: Sender<Event>, rom: &'static str) {
    std::thread::spawn(move || {
      loop {
        match srv_socket.accept() {
          Ok((socket, addr)) => {
            let client = Client { id: ClientId(addr), socket, rom };
