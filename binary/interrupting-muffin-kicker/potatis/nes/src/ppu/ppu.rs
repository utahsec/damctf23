    ...
    for _ in 0..ppu_cycles_to_tick {
      match self.state.next(self.rendering_enabled) {
        ...
        (Phase::EnteringVblank, 1, _) => {
            self.in_vblank = true;
            let dst = self.frame.nametable_mut();
            let bytes = crate::frame::NAMETABLE_WIDTH * crate::frame::NAMETABLE_HEIGHT;
            dst[0..bytes].copy_from_slice(&self.vram.nametables()[0][0..bytes]);

            self.frame.sprite0_y = self.oam[0];
            self.frame.sprite0_x = self.oam[3];
            self.frame.sprite0_tile = self.oam[1];
        },
